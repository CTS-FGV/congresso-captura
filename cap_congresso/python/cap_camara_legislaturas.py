...
class Legislaturas_Senado(Capture):
    ...
    def fill_entry_legislaturas(self, legislatura, entry_columns):
        ...
        sigla_casa = 'CD'
        id_legislatura = utils.generate_hash(str(id_api),sigla_casa)

        entry = copy.deepcopy(entry_columns)
        entry['id_legislatura'] = id_legislatura
        entry['id_api'] = int(id_api)
        entry['numero_legislatura'] = int(id_api)
        entry['sigla_casa'] = sigla_casa
        entry['data_inicio'] = tgd(legislatura,'dataInicio')
        entry['data_fim'] = tgd(legislatura,'dataFim')
        entry['data_captura'] = datetime.datetime.today()
        entry['api_captura'] = "CD"
...
if __name__ == '__main__':
    ...
    url = 'https://dadosabertos.camara.leg.br/api/v2/legislaturas/?itens=1000'
    data_source = utils.get_json(url)
    iterable = data_source['dados']
