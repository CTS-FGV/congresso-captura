...
####################################################################################################
# Gera índice de fidelidade governamental de partidos por mês
#
#
####################################################################################################
...
def main():
    ...
    script = "r_camdep_fid_gov_part_mes.py"
    ...
    # LOG
    st = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
    detalhes = "Limpando dados da tabela r_camdep.fid_gov_part_mes"
    cursor.execute("INSERT INTO c_camdep.camdep_capture_log (capnum, script, datahora, detalhes) VALUES ({0}, '{1}', '{2}', '{3}')".format(nextcapnum,script,st,detalhes))
    conn.commit()

    # EXECUÇÃO
    cursor.execute("DELETE FROM r_camdep.fid_gov_part_mes")

    # LOG
    st = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
    detalhes = "Concluída limpeza da tabela r_camdep.fid_gov_part_mes"
    cursor.execute("INSERT INTO c_camdep.camdep_capture_log (capnum, script, datahora, detalhes) VALUES ({0}, '{1}', '{2}', '{3}')".format(nextcapnum,script,st,detalhes))
    conn.commit()
    ...
    # LOG
    st = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
    detalhes = "Inserindo dados na r_camdep.fid_gov_part_mes"
    cursor.execute("INSERT INTO c_camdep.camdep_capture_log (capnum, script, datahora, detalhes) VALUES ({0}, '{1}', '{2}', '{3}')".format(nextcapnum,script,st,detalhes))
    conn.commit()

    # EXECUÇÃO
    sql = """
    INSERT INTO r_camdep.fid_gov_part_mes
    SELECT
        CONCAT(EXTRACT(YEAR FROM d.datavotacao)::TEXT,'-',LPAD(EXTRACT(MONTH FROM d.datavotacao)::TEXT,2,'0')) anomes,
        d.partido,
    ...
    # LOG
    st = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
    detalhes = "Concluída inserção de dados na tabela r_camdep.fid_gov_part_mes"
    cursor.execute("INSERT INTO c_camdep.camdep_capture_log (capnum, script, datahora, detalhes) VALUES ({0}, '{1}', '{2}', '{3}')".format(nextcapnum,script,st,detalhes))
